...
def generate_eth_cancel_change_eth_address_intent_vectors():
    """Generate test vectors for ETH canceling change ETH address intents for all 5 production actors"""
    cancel_vectors = []
    ...
    for i in range(num_actors):
        ...
        next_eth_address = next_actor["eth_address"]
        pq_fingerprint = current_actor["pq_fingerprint"]
        eth_private_key = next_actor["eth_private_key"]

        pq_nonce = 3  # PQ nonce for cancel operation
        eth_nonce = 1  # ETH nonce for cancel operation (next actor's nonce)
...
def main():
    ...
    try:
        ...
        # Save to JSON file
        output_file = project_root / "test/test_vectors/production/change_eth/change_eth_address_cancel_eth_vectors.json"
