...
def generate_change_eth_address_intent_vectors():
    ...
    # Define the cycling pattern: kyle -> luke -> marie -> nancy -> oscar -> kyle
    actor_cycle = ["kyle", "luke", "marie", "nancy", "oscar"]
    
    for i in range(5):  # Changed from 10 to 5 since we have 5 production actors
        current_actor_name = actor_cycle[i]
        next_actor_name = actor_cycle[(i + 1) % 5]  # Wrap around to kyle for the last one
