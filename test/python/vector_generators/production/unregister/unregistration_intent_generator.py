...
import sys
sys.path.append(str(Path(__file__).resolve().parents[3]))  # Add python directory to path
...
OUTPUT_PATH = Path(__file__).resolve().parents[5] / "test/test_vectors/production/unregister/unregistration_intent_vectors.json"
...
def main():
    ...
    vectors = []
    for actor_name in ["kyle", "luke", "marie", "nancy", "oscar"]:
