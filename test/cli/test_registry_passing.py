import subprocess
from eth_account import Account
import os
import sys

# Add the python-ref directory to the path
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../../ETHFALCON/python-ref')))
from falcon_epervier import HEAD_LEN, SALT_LEN, decompress
from common import falcon_compact, q
from polyntt.poly import Poly

def parse_signature_like_cli(sig_path):
    with open(sig_path, 'r') as f:
        sig_hex = f.read().strip()
    sig = bytes.fromhex(sig_hex)
    salt = sig[HEAD_LEN:HEAD_LEN + SALT_LEN]
    enc_s = sig[HEAD_LEN + SALT_LEN:-512*3]
    s = decompress(enc_s, 666*2 - HEAD_LEN - SALT_LEN, 512*2)
    mid = len(s) // 2
    s = [elt % q for elt in s]
    s1, s2 = s[:mid], s[mid:]
    s1_compact = falcon_compact(s1)
    s2_compact = falcon_compact(s2)
    s2_inv_ntt = Poly(s2, q).inverse().ntt()
    hint = 1
    for elt in s2_inv_ntt:
        hint = (hint * elt) % q
    return {
        'salt': salt.hex(),
        'cs1': s1_compact,
        'cs2': s2_compact,
        'hint': hint
    }

def get_public_key():
    with open('../../ETHFALCON/python-ref/public_key.pem', 'r') as f:
        content = f.read()
        import re
        match = re.search(r'pk\s*=\s*(\d+)', content)
        if match:
            pk_value = int(match.group(1))
            return [pk_value, 0]
    return None

def main():
    # 1. Use the default Foundry/Anvil private key
    eth_private_key = "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"
    eth_account = Account.from_key(eth_private_key)
    eth_address = eth_account.address

    # 2. Generate the intent message
    nonce = 0
    intent_message = f"Register Epervier Key{eth_address}{nonce}".encode()

    # 3. Sign the message using your CLI
    subprocess.run([
        "python", "../../ETHFALCON/python-ref/sign_cli.py", "sign",
        "--privkey", "../../ETHFALCON/python-ref/private_key.pem",
        "--data", intent_message.hex()
    ], check=True)

    # 4. Test recovery to get the actual address
    print("Testing recovery to get the actual address...")
    result = subprocess.run([
        "python", "../../ETHFALCON/python-ref/sign_cli.py", "recoveronchain",
        "--signature", "../../ETHFALCON/python-ref/sig",
        "--data", intent_message.hex(),
        "--pubkey", "../../ETHFALCON/python-ref/public_key.pem",
        "--contractaddress", "0x5FbDB2315678afecb367f032d93F642f64180aa3",
        "--rpc", "http://localhost:8545"
    ], capture_output=True, text=True)
    
    recovered_address = result.stdout.strip().split(": ")[-1]
    print(f"Recovered address: {recovered_address}")
    
    # 5. Now create the intent message with the recovered address
    intent_message_with_recovered = f"Register Epervier Key{recovered_address}{nonce}".encode()
    print(f"New intent message: {intent_message_with_recovered}")
    
    # 6. Sign the new message
    subprocess.run([
        "python", "../../ETHFALCON/python-ref/sign_cli.py", "sign",
        "--privkey", "../../ETHFALCON/python-ref/private_key.pem",
        "--data", intent_message_with_recovered.hex()
    ], check=True)

    # 7. Parse the signature using your CLI logic
    sig_components = parse_signature_like_cli('../../ETHFALCON/python-ref/sig')
    public_key = get_public_key()
    if not public_key:
        print("Failed to get public key")
        return

    # 8. Submit the transaction using cast
    registry_address = "0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9"
    rpc_url = "http://localhost:8545"
    command = [
        "cast", "send", registry_address,
        "submitRegistrationIntent(bytes,bytes,uint256[],uint256[],uint256,uint256[2],uint256)",
        f"0x{intent_message_with_recovered.hex()}",
        f"0x{sig_components['salt']}",
        str(sig_components['cs1']),
        str(sig_components['cs2']),
        str(sig_components['hint']),
        str(public_key),
        str(nonce),
        "--rpc-url", rpc_url,
        "--private-key", eth_private_key
    ]
    print(" ".join(command))
    result = subprocess.run(command, capture_output=True, text=True)
    print("Return code:", result.returncode)
    print("STDOUT:", result.stdout)
    print("STDERR:", result.stderr)

if __name__ == "__main__":
    main() 