...
contract MessageParserContract is IMessageParser {
     ...
     */
    function parseETHAddressFromETHUnregistrationConfirmationMessage(bytes memory message) external pure returns (address fingerprintAddress) {
        bytes memory pattern = "Confirm unregistration from Epervier Fingerprint ";
     ...
     */
    function extractEthNonce(bytes memory message, uint8 messageType) external pure returns (uint256 ethNonce) {
        if (messageType == 0) {
     ...
     */
    function parsePQRemoveRegistrationIntentMessage(bytes memory message, bytes32 domainSeparator) external pure returns (
        address ethAddress,
        uint256 pqNonce
    ) {
        // Extract domain separator (first 32 bytes) and validate it
     ...
     */
    function extractPQNonceFromRemoveMessage(bytes memory message) external pure returns (uint256 pqNonce) {
     ...
     */
    function validateDomainSeparator(bytes memory message, bytes32 expectedDomainSeparator) external pure returns (bool) {
        require(message.length >= 32, "Message too short for domain separator");
     ...
     * Expected format: DOMAIN_SEPARATOR + "Confirm changing bound ETH Address for Epervier Fingerprint from " + oldEthAddress + " to " + newEthAddress + pqNonce
     */
    function validatePQChangeETHAddressConfirmationMessage(bytes memory message) external pure returns (bool) {
        bytes memory pattern = "Confirm changing bound ETH Address for Epervier Fingerprint from ";
        return findPattern(message, pattern, true) != type(uint).max;
    }
     ...
     */
    function validateETHUnregistrationIntentMessage(bytes memory message) external pure returns (bool) {
        bytes memory pattern = "Intent to unregister from Epervier Fingerprint ";
        return findPattern(message, pattern, false) != type(uint).max;
    }
    ...
    function extractPQSalt(bytes memory message, uint8 messageType) external pure returns (bytes memory salt) {
        if (messageType == 0) {
            ...
            // Extract salt (40 bytes after ethNonce)
            salt = new bytes(40);
            for (uint256 i = 0; i < 40; i++) {
                salt[i] = message[32 + patternLength + 32 + i]; // DOMAIN_SEPARATOR + pattern + ethNonce + offset
            }
            return salt;
        } else if (messageType == 1) {
